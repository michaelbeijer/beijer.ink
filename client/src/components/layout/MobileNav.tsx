...
export function MobileNav({ activeView, onChangeView, hasNotebook, hasNote }: MobileNavProps) {
  return (
    <div className="flex items-center justify-around bg-slate-900 border-t border-slate-800 py-2 lg:hidden">
      <button
        onClick={() => onChangeView('sidebar')}
        className={`flex flex-col items-center gap-0.5 px-4 py-1 rounded-lg transition-colors ${
          activeView === 'sidebar' ? 'text-blue-400' : 'text-slate-500'
        }`}
      >
      ...
      <button
        ...
        className={`flex flex-col items-center gap-0.5 px-4 py-1 rounded-lg transition-colors ${
          activeView === 'notes' ? 'text-blue-400' : 'text-slate-500'
        } disabled:opacity-30`}
      >
      ...
      <button
        ...
        disabled={!hasNote}
        className={`flex flex-col items-center gap-0.5 px-4 py-1 rounded-lg transition-colors ${
          activeView === 'editor' ? 'text-blue-400' : 'text-slate-500'
        } disabled:opacity-30`}
      >
