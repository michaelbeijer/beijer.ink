...
export function NoteListPanel({ notebookId, selectedNoteId, onSelectNote }: NoteListPanelProps) {
  ...
  if (!notebookId) {
    return (
      <div className="h-full flex items-center justify-center bg-slate-950 border-r border-slate-800">
      ...
      <div className="flex-1 overflow-y-auto">
                  ...
                  {note.tags.length > 0 && (
                    <div className="flex gap-1">
                      {note.tags.slice(0, 2).map((nt) => (
                        <span
                          key={nt.tagId}
                          className="text-xs px-1.5 py-0.5 rounded-full bg-slate-800 text-slate-400"
                        >
                          {nt.tag.name}
                        </span>
                      ))}
                      {note.tags.length > 2 && (
                        <span className="text-xs text-slate-600">+{note.tags.length - 2}</span>
                      )}
                    </div>
                  )}
                </div>
        ...
        {notes.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 text-slate-500">
            ...
            <button onClick={handleCreate} className="text-blue-400 text-sm hover:underline mt-1">
              Create one
            </button>
          </div>
        )}
